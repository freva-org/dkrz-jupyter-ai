...
class MultiEnvAuthStrategy(BaseModel):
    ...
    def _validate_keyword_params(self) -> Self:
        if self.keyword_params:
            if len(self.keyword_params) != len(self.names):
                raise ValidationError("keyword_params need to be equal in length to names")
        else:
            self.keyword_params = [None] * len(self.names)
        return self
