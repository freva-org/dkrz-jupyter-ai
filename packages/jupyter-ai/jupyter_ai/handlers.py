...
OAUTH_CLIENT_ID = "nextgems"
OAUTH_AUTH_URL = "https://nextgems.dkrz.de/api/freva-nextgen/auth/v2/login"
OAUTH_TOKEN_URL = "https://nextgems.dkrz.de/api/freva-nextgen/auth/v2/token"

# Store state and code_verifier in memory for demo
pending_states = {}


class OAuthStartHandler(JupyterHandler):
    def get(self):
        ...
        redirect_uri = url_path_join(self.request.protocol + "://" + self.request.host, "api/ai/oauth/callback")
        auth_url = (
...
class OAuthCallbackHandler(JupyterHandler):
    def get(self):
        ...
        if not code or not state:
            raise tornado.web.HTTPError(400, "Missing code or state")
        ...
        os.environ["FREVA_TOKEN_JSON"] = json.dumps(token_data)
        with open(os.path.join(os.path.expanduser("~"), ".freva_token.json"), "w") as f:
            json.dump(token_data, f)

        # Store in session or pass directly to frontend
        self.set_secure_cookie("oauth_token", access_token)

        # Redirect back to the extension UI page
        self.redirect("/lab?oauth=success")
